...
const CFG_KEY = 'team-based-robot';
...
function processFile(path, name, stat, settings) {
  ...
  if (stat.isFile() && stat.size < settings.maxFileSize) {
    ...
    if (isRobotFile(fileContent, fullPath, settings)) {
      ...
      console.log("robotInfo ", parsedRobotInfo)
      return Promise.resolve();
    }
...
function readConfig(){
  ...
  settings.showArguments = atom.config.get(`${CFG_KEY}.showArguments`) || true;
  ...
  settings.suggestArguments = true //atom.config.get(`${CFG_KEY}.suggestArguments`);
  settings.includeDefaultArguments = false //atom.config.get(`${CFG_KEY}.includeDefaultArguments`);
  ...
  settings.removeDotNotation = true //atom.config.get(`${CFG_KEY}.removeDotNotation`);
  settings.processLibdocFiles = true //atom.config.get(`${CFG_KEY}.processLibdocFiles`);
  settings.showLibrarySuggestions = false //atom.config.get(`${CFG_KEY}.showLibrarySuggestions`);
  settings.pythonExecutable = 'python' //atom.config.get(`${CFG_KEY}.pythonExecutable`);
};
...
var provider = {
  ...
  load() {
    ...
    // Config change handler => read package config, reload auto complete data
  ...
  reload() {
    console.log("Reload Auto Complete");
    reloadAutocompleteData()
  },
