...
var nameRegexp = namedRegexp(/^(:<name>[^\t]+)\t?$/);
...
var argumentRegexp = namedRegexp(/[$@&]{(:<argument>[^\t=]+)}(=(:<defaultValue>.*?)((\t)|$))?/gi);
...
*  }
*/
var parse = function(fileContentOrig, includeContent) {
  var keywords;
  var testCases;
  var libraries;
  var keywordsMatch;
  ...
  keywords = keywordsSection ? parseKeywordOrTestCase(keywordsSection, true, includeContent):[]
...
var parseKeywordSelection = function(content) {
  var fileContentTabs = content.replace(tabRegexp, '\t');
  var fileContent = fileContentTabs.replace(singleLineCommentRegexp, '');
  fileContent = fileContent.replace(emptyLineRegexp, '$1');
  fileContent = `*** Keywords ***\n ${fileContent}`

  var sections = getSections(fileContent);
  var keywordsSection = sections['keywords'] || sections['keyword'];
  var keywords = keywordsSection ? parseKeywordOrTestCase(keywordsSection, true, true) : []
  return keywords
}

var parseKeywordOrTestCase = function(section, processArguments, includeContent){
  ...
  while(match = individualKeywordOrTestCaseRegexp.exec(section)){
    ...
    if(processArguments){
      ...
      if(argumentsMatch){
        ...
        while(argMatch = argumentRegexp.execGroups(keywordArgsStr)){
          if(argMatch['defaultValue']){
            arguments.push(`${argMatch['argument']}=${argMatch['defaultValue']}`);
          } else{
    ...
    if (includeContent) {
      ...
      if (documentation != "") {
        ...
        cleanContent = cleanContent.replace(/\[Documentation\]/, '')
        cleanContent = cleanContent.replace(documentation, '')
        cleanContent = cleanContent.replace(/^\n|\r\n/, '')
        cleanContent = cleanContent.replace(/^\t\t/, '')
      }
      data.content = content
      data.contentWithoutDoc = cleanContent
    }
    result.push(data);
  }
  return result;
...
var parseResources = function(settingsSection){
  ...
  while(match = resourceRegexp.exec(settingsSection)){
    ...
    if(resourcePath){
      resources.push({path: resourcePath.trim()})
    }
  }
  return resources
}
...
module.exports = {
  parse: parse,
  isRobot: isRobot,
  parseKeywordSelection: parseKeywordSelection
}
